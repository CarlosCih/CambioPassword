...
{
    $token = "NULL"; // Esto inicializa el token como una cadena de texto "NULL"
    ...
    $stmt->close();
    return $token; // Devuelve el token original
}
...
function cambioPassword($conn){
    if (isset($_POST['cambio_contraseña'])) {
        ...
        if ($result->num_rows == 1) {
            ...
            // Cifrar la nueva contraseña (usando el mismo método que al registrar el usuario)
            $contrasenaCifrada = cifrarContrasena($password);
